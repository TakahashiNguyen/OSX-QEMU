...
args=(
  ...
  -m "$ALLOCATED_RAM" 
  # -device nec-usb-xhci,id=xhci -device usb-tablet -device usb-kbd -global nec-usb-xhci.msi=off
  ...
  # -drive if=pflash,format=raw,readonly=on,file="$REPO_PATH/$OVMF_DIR/OVMF_combined.fd"
  ...
  -rtc base=localtime -bios "OVMF_combined.fd"
